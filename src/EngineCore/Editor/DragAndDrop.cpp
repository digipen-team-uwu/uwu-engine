...
{
  bool updateAtlasPage = false; //only set to true if something succeeded in loading in
  for(int i = 0; i < count; ++i)
  {
    const std::string path = paths[i];
    const bool jsonCheck = path.find(".json") != path.npos || path.find(".JSON") != path.npos;
    const bool pngCheck = path.find(".png") != path.npos || path.find(".PNG") != path.npos;
    if(jsonCheck)
    {
      EntityFactory::CreateObject(path);
      updateAtlasPage = true;
    }
    else if(!pngCheck) //not a json nor png file
        TraceLogger::Log(TraceLogger::WARNING, "Dropped file is not a json/JSON nor png/PNG file!!!\n");
    else //if png
    {
