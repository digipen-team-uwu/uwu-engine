...
void GLSLShader::SetUniform(GLchar const* name, GLboolean val) const {
    GLint loc = glGetUniformLocation(pgm_handle, name);
    if (loc >= 0) {
        glUniform1i(loc, val);
    }
    else {
      TraceLogger::Log(TraceLogger::ERROR) << "Uniform variable " << name << " doesn't exist" << std::endl;
    }
}

void GLSLShader::SetUniform(GLchar const* name, GLint val)const {
    GLint loc = glGetUniformLocation(pgm_handle, name);
    if (loc >= 0) {
        glUniform1i(loc, val);
    }
...
void GLSLShader::SetUniform(GLchar const* name, GLfloat val) const {
    GLint loc = glGetUniformLocation(pgm_handle, name);
    if (loc >= 0) {
        glUniform1f(loc, val);
    }
...
void GLSLShader::SetUniform(GLchar const* name, GLfloat x, GLfloat y)const {
    GLint loc = glGetUniformLocation(pgm_handle, name);
    if (loc >= 0) {
        glUniform2f(loc, x, y);
    }
...
void GLSLShader::SetUniform(GLchar const* name, GLfloat x, GLfloat y, GLfloat z)const {
    GLint loc = glGetUniformLocation(pgm_handle, name);
    if (loc >= 0) {
...
void
GLSLShader::SetUniform(GLchar const* name, GLfloat x, GLfloat y, GLfloat z, GLfloat w)const {
    GLint loc = glGetUniformLocation(pgm_handle, name);
    if (loc >= 0) {
...
void GLSLShader::SetUniform(GLchar const* name, glm::vec2& val)const {
    GLint loc = glGetUniformLocation(pgm_handle, name);
    if (loc >= 0) {
...
void GLSLShader::SetUniform(GLchar const* name, glm::uvec2& val)const {
    GLint loc = glGetUniformLocation(pgm_handle, name);
    if (loc >= 0) {
        glUniform2f(loc, static_cast<GLfloat>(val.x), static_cast<GLfloat>(val.y));
    }
    else {
      TraceLogger::Log(TraceLogger::ERROR) << "Uniform variable " << name << " doesn't exist" << std::endl;
    }
...
void GLSLShader::SetUniform(GLchar const* name, glm::vec3& val)const {
    GLint loc = glGetUniformLocation(pgm_handle, name);
    if (loc >= 0) {
...
void GLSLShader::SetUniform(GLchar const* name, glm::vec4& val)const {
    GLint loc = glGetUniformLocation(pgm_handle, name);
    if (loc >= 0) {
...
void GLSLShader::SetUniform(GLchar const* name, glm::mat3& val) const {
    GLint loc = glGetUniformLocation(pgm_handle, name);
    if (loc >= 0) {
...
void GLSLShader::SetUniform(GLchar const* name, const glm::mat4& val) const {
    GLint loc = glGetUniformLocation(pgm_handle, name);
    if (loc >= 0) {
...
void GLSLShader::PrintActiveAttribs() const {
    ...
    TraceLogger::Log(TraceLogger::DEBUG) << "Index\t|\tName\n";
    ...
    for (GLint i = 0; i < num_attribs; ++i) {
        ...
        TraceLogger::Log(TraceLogger::DEBUG) << loc << "\t\t" << pname << std::endl;
    }
    TraceLogger::Log(TraceLogger::DEBUG) << "----------------------------------------------------------------------\n";
    delete[] pname;
...
void GLSLShader::PrintActiveUniforms() const {
    ...
    TraceLogger::Log(TraceLogger::DEBUG) << "Location\t|\tName\n";
    TraceLogger::Log(TraceLogger::DEBUG) << "----------------------------------------------------------------------\n";
