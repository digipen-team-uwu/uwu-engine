...
AtlasModule::AtlasModule(ISpace* p) : System(p)
{
...
void AtlasModule::report_result(const rect_wh& result_size, const std::vector<rect_type>& rectangles)
{
...
const std::vector<rect_type>& AtlasModule::GetRectangles()
{
  return rectangles;
}

const AtlasModule::RawData& AtlasModule::GetRawData(const std::string& filePath)
{
  return filePath_to_RawData.at(filePath);
}


void AtlasModule::CheckingLayer()
{
  if (is_first_pass_)
  {
...
void AtlasModule::Atlasing(std::vector<RawData>& data_)
{
...
bool AtlasModule::RawData::operator()(const RawData& r1, const RawData& r2)
{
  return r1.area < r2.area;
}

void AtlasModule::LoadAtlasPage(GLboolean isTwice_)
{
  if (isTwice_)
  {
    ClearData();
...
void AtlasModule::LoadRawData(const std::vector<std::string>& files)
{
...
void AtlasModule::LoadRawData(int total...)
{
  va_list args;
  va_start(args, total);
...
void AtlasModule::SetAtlasData(EntityID ID, unsigned int accessID)
{
...
void AtlasModule::GetAtlasData(const char* filepath, glm::vec2* scale, glm::vec2* uvs, unsigned* layer)
{
...
void AtlasModule::SetAtlasLayers(EntityID ID, AtlasLayer new_layer)
{
  atlas_layers[ID] = new_layer;
}

const AtlasLayer AtlasModule::GetAtlasLayers(EntityID ID)
{
  return atlas_layers[ID];
}

const std::vector<AtlasLayer>& AtlasModule::GetArrayAtlasLayer()
{
  return atlas_layers;
}

const GLuint AtlasModule::GetTextureID()
{
  return texID;
}

const glm::vec2 AtlasModule::GetAtlasUV(EntityID ID)
{
  return atlas_uv[ID];
}

const glm::vec2 AtlasModule::GetAtlasScale(EntityID ID)
{
  return atlas_scale[ID];
}

const std::vector<glm::vec2>& AtlasModule::GetArrayAtlasUV()
{
  return atlas_uv;
}

const std::vector<glm::vec2>& AtlasModule::GetArrayAtlasScale()
{
  return atlas_scale;
}

glm::vec2 AtlasModule::GetBrayanTexture(const std::string& filePath)
{
  int image_width;
  int image_height;
  int number_channels;
